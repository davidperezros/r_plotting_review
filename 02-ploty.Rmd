...
data_ecoicop_2$parent <- get_label(data_ecoicop_2$`Subgrupos.de.gasto.(3.dígitos).Codigo`, label = data_ecoicop_1$`Grupos.de.gasto.(2.dígitos)`)
```
...
parents<-c("", rep(aux[1], length(aux) - 1), data_ecoicop_2$parent)
...
fig <- plot_ly(
  ...
  values = as.integer(c(data_ecoicop_1$Valor, data_ecoicop_2$Valor),0),
  maxdepth = 2,
    branchvalues='total',
...
```{r, webshot = "webshot", out.width = "80%",fig.cap="sgf"}
fig
```
...
2. Usando `ineapir` extraemos los datos usando el servicio API del INE. Ejecutando  `get_metadata_table_varval(50902)` vemos que debemos filtrar por Índice general, y variación anual.
...
2. Usando `ineapir` extraemos los datos usando el servicio API del INE. Ejecutando  `get_metadata_table_varval(2915)` vemos que debemos filtrar por  todos municipios
...
2. Usando `ineapir` extraemos los datos usando el servicio API del INE. Ejecutando  `get_metadata_table_varval(66257)` vemos que debemos filtrar por  ambos sexos
